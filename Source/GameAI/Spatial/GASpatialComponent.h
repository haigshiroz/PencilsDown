...
class UGASpatialComponent : public UActorComponent
{
	...
	// Tick to track the player's last time moved 
	virtual void TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction) override;

	// Used to intiialize values
	...
	// Hysteresis value to add to the score of the last chosen destination
	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	float HysteresisModifier;
	...
	UFUNCTION(BlueprintCallable)
	bool ChoosePosition(bool PathfindToPosition, bool Debug, bool HighlightDestination);

	void EvaluateLayer(const FFunctionLayer& Layer, const FGAGridMap& DistanceMap, FGAGridMap& GridMap) const;


	// Additional info to make decisions

	FVector LastPlayerLocation;

	float TimeSincePlayerMoved;
