...
bool AGAGridActor::RefreshDebugTexture(const FCellRef& Destination, bool HighlightDestination)
{
	...
	*/
	...
	if (DebugMeshComponent)
	{
		...
		{
			float MaxValue;
			FCellRef CellWithMaxValue;
			DebugGridMap.GetMaxValue(MaxValue, CellWithMaxValue, BIG_NUMBER);
			...
			for (int32 Y = 0; Y < YCount; Y++)
			{
				for (int32 X = 0; X < XCount; X++)
				{
					...
					const int32 DestX = Destination.X;
					const int32 DestY = Destination.Y;
					const bool bIsInDestinationBlock = FMath::Abs(X - DestX) <= 3 && FMath::Abs(Y - DestY) <= 3;
					if (HighlightDestination && bIsInDestinationBlock)
					{
						// Draw a pixel
						RawImageData[Index] = 255;   // Blue
						RawImageData[Index + 1] = 255;   // Green
						RawImageData[Index + 2] = 255;   // Rred
