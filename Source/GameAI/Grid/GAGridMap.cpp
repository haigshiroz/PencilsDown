...
bool FGAGridMap::GetMaxValue(float& MaxValueOut, FCellRef& CellWithMaxValueOut, float IgnoreThreshold) const
{
	if (IsValid())
	{
		...
		int32 Y = MaxIndex / XCount;
		int32 X = MaxIndex % XCount;
		CellWithMaxValueOut = FCellRef(X, Y);
